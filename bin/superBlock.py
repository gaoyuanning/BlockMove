...
def todict(op):
    ...
    #print temp
    return temp
#Python list 转 Java ArrayList
def CovertPlistTOJlist(pList):
    ret = ArrayList()
...
def solveBlock(s,length):
    ...
    while(op!={}):
        ...
        if judgeBlock(k.name,length)==0:
            ...
            #return k.path
            return CovertPlistTOJlist(k.path)
        flag=False
        for i in cl.keys():
            if i.name==k.name:
                if cl[i]>op[k]:
        ...
        for i in range(len(res)):     
            if res[i] in getName(op.keys()) :
                ...
                if pri[i]<op[tNode]:
                    ...
                    del nNode.path[:]
                    for j in range(len(k.path)):
                        nNode.path.append(k.path[j])
                    ...
                    if judgeBlock(nNode.name,length)==0:
                        ...
                        #return nNode.path
                        return CovertPlistTOJlist(nNode.path)
                    #print k.path
